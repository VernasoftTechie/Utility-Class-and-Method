...
| A17 | **Dynamic BAPI call** → `CALL FUNCTION fname PARAMETER-TABLE pt EXCEPTION-TABLE et` where `pt TYPE abap_func_parmbind_tab`, `et TYPE abap_func_excpbind_tab` (line `( name = 'OTHERS' value = 1 )`). `kind` constants (type-pool `ABAP`): `abap_func_exporting` 'E' (→ FM IMPORTING), `abap_func_importing` 'I' (← FM EXPORTING), `abap_func_tables` 'T', `abap_func_changing` 'C'. Wrap the call in `TRY … CATCH cx_root` (dyn-call cx: illegal func / param missing / illegal param) **and** check `sy-subrc` after. Build the `value` refs with `CREATE DATA` and keep them alive (a table of `REF TO data`) for the duration of the call. |
| A18 | **FM interface introspection** without guessing an FM signature: `SELECT parameter, paramtype FROM fupararef WHERE funcname = @f AND r3state = 'A'`. `paramtype`: `I` import / `E` export / `C` changing / `T` tables / `X` exception. For `RETURN`, bind your own `bapiret2_t` (T) or `bapiret2` (E) receiver — don't rely on the FM's declared type. |
...
| A26 | **`to_upper( )`/`to_lower( )` do not preserve the actual's specific DDIC type** — `DATA(lv_x) = to_upper( iv_y ).` where `iv_y TYPE sobj_name` infers `lv_x` as generic `STRING`, not `sobj_name`. Passing `lv_x` **by reference** into a formal still declared as the concrete type (e.g. `iv_obj_name TYPE sobj_name`) then fails *"… is not type-compatible with formal parameter …"* — a fresh trigger of the existing by-ref rule (T2). Literals and constants (`'TABL'`, `space`) are unaffected — the compiler builds a compatible anonymous temp for those, only plain variables are checked strictly. **Fix:** pre-declare the local with the real target type and assign via `=` (`DATA lv_x TYPE sobj_name. lv_x = to_upper( iv_y ).`) — assignment converts, by-ref parameter binding does not. Hit in `ZCL_AB_V1_UT_DEPDEL~ddic_dependencies`/`add_table_deps`/`add_dtel_deps` (6 occurrences, all `to_upper`-derived locals re-fed into other methods' `iv_obj_name`/`iv_parent_obj`/`is_customer_object` calls). |
| A27 | **Unqualified interface-method call on a class-typed variable failed to activate**: `DATA(lo) = NEW zcl_ab_v1_ut_depdel( ). lo->ddic_dependencies( ... ).` gave *"Method DDIC_DEPENDENCIES is unknown or PROTECTED or PRIVATE"* at activation on this system, even though `ZCL_AB_V1_UT_DEPDEL` only reaches the method via `INTERFACES zif_ab_v1_ut_depdel.` (per the ABAP OO rule, that should make it a normal unqualified component of the class too). Confirmed fix: qualify explicitly — `lo->zif_ab_v1_ut_depdel~ddic_dependencies( ... )`. Every other demo report in this repo gets its object via the **facade** (`zcl_ab_v1_ut=>transport( )` etc.), which returns an **interface-typed** reference, so unqualified calls there were never exercised — this is the first demo calling a non-facade area class directly (`NEW zcl_x( )`) and hitting it. **Rule going forward:** when a demo/report instantiates a Gated-area class directly, always qualify the interface-method call (`lo->zif_..._area~method( )`), don't rely on the short form. |
...
| _(2026-09-13)_ | `ZCL_AB_V1_UT_DEPDEL` activation: 6× *"not type-compatible with formal parameter"* — `to_upper( )` returns generic `STRING`, inline `DATA(lv_x) = to_upper(...)` then fails by-ref binding to a concrete-typed formal (A26). Fixed: pre-declared locals (`lv_object TYPE trobjtype`, `lv_obj_name`/`lv_dtel`/`lv_dom TYPE sobj_name`) assigned via `=`. |
| _(2026-09-13)_ | `ZAB_V1_UT_DEMO_DEPDEL` activation: *"Method DDIC_DEPENDENCIES is unknown or PROTECTED or PRIVATE"* persisted even after the class activated clean — unqualified interface-method call on a class-typed `NEW zcl_x( )` reference (A27). Fixed: qualified the call `lo->zif_ab_v1_ut_depdel~ddic_dependencies( ... )`. |
